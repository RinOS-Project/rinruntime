...
The C sources are built with only `include/` on their public include path.
The build does not add the OS-Core root, a sibling library, or a private
generated-header directory.  A service implementation must be injected by a
separate authenticated adapter; the public library never substitutes a
successful result when that adapter is absent.
