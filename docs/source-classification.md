...
| `render_context.hpp` / `render_context.cpp` | A/B: public runtime adapter | Frame/context APIs and the borrowed `setSystemUiFontLoader()` callback are public; locale selection, font bytes, resource paths, and filesystem ownership remain in the private application/provider adapter. |
...
| Firewall rule engine, application identity, profile table, detached conntrack snapshot query | A/B: public runtime model | Fixed-width Firewall ABI validation/evaluation, bounded application/profile operations, and filtering of a caller-owned immutable connection snapshot live in `src/firewall` with SDK/RinRuntime public headers. The public application-context validator accepts a zero package generation only with the descriptive system-process flag; the private kernel adapter derives that flag from kernel-owned launch identity. Packet hooks, identity provenance, live conntrack observation/state, factory policy, interface identity authority, persistence, and firewalld authorization remain private OS-Core. |
